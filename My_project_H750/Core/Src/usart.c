...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    if (huart->Instance == USART1) // 确保是 USART1
    {
        ...
        if (Size > 0 && Size < BUFFER_SIZE)  // 确保收到数据
        {
            ...
            if(strncmp((char*)uart1_buf_recv, "MQTT time", 9) == 0){
              sendStringData(&huart1, (char*)"ctime?"); //发送当前时间请求  
            }

            if (strncmp((char*)uart1_buf_recv, "stime", 5) == 0) {   //获取时间消息
              sscanf((char*)uart1_buf_recv + 5, "%2hhu%2hhu%2hhu%2hhu:%2hhu",
                &start_time.year, &start_time.month, &start_time.day,
                &start_time.hour, &start_time.minute);
                time_received_flag = 1; // 时间接收标志
                lv_label_set_text_fmt(uart1_label, "starttime: %s", uart1_buf_recv);
            }
            if(strncmp((char*)uart1_buf_recv, "ctime", 5) == 0){
              sscanf((char*)uart1_buf_recv + 5, "%2hhu%2hhu%2hhu%2hhu:%2hhu",
              ...
              // LCD_DisplayString(0, 25, (char*)uart1_buf_recv);
              lv_label_set_text_fmt(uart1_label, "uart1: %s", uart1_buf_recv);
            }
        }
        else{
